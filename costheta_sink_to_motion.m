...
unit_vector_primary_sink = load(fullfile([d '/data'], ['unit_vector_primary_sink_', output_name, '.mat']));
unit_vector_primary_sink = unit_vector_primary_sink.s_unit;
...
%% %%
costheta_primary_sink = zeros(nt,1);

for k = 1:nt
    
    costheta_primary_sink(k,1) = dot(unit_vector_primary_sink(k,:), track_diff(k,:)) ./...
        (norm(unit_vector_primary_sink(k,:)) * norm(track_diff(k,:)));
...
['costheta_primary_sink_', output_name,'.mat']), ...
'costheta_primary_sink');
